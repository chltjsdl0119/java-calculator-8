...
### 1. 입력
- [x] 사용자는 camp.nextstep.edu.missionutils.Console의 readLine()을 통해 문자열을 입력받는다.
- [x] 입력 문자열은 '//' 와 '\n' 사이에 커스텀 구분자를 정의할 수 있다. (ex: "//;\n1;2;3"과 같이 입력할 경우, 커스텀 구분자는 세미콜론(;)이다.)

### 2. 구분자 처리
- [x] 기본 구분자는 쉼표(,)와 콜론(:)이다.
- [x] 커스텀 구분자가 존재할 경우, 해당 구분자를 사용한다.
- [x] 입력 문자열에서 구분자를 기준으로 숫자를 분리한다.
- [x] 잘못된 구분자 형식을 입력할 경우 IllegalArgumentException을 발생시킨다.
...
### 4. 계산
- [x] 분리된 모든 숫자를 더한 결과를 반환한다.
- [x] 빈 문자열일 경우, 0을 반환한다.

### 5. 출력
- [x] 계산된 결과를 "결과 : {sum}" 형태로 출력한다.

### 6. 예외 처리
- [x] 입력이 잘못된 경우 IllegalArgumentException을 발생시키고 애플리케이션을 종료한다.
- [x] System.exit()를 사용하지 않고, 예외를 던지는 방식으로 프로그램을 종료한다.
